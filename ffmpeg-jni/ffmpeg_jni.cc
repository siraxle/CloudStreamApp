...
// Jellyfin media3 ≥ 1.4 passes a SimpleDecoderOutputBuffer before outputData so
// the JNI can resize the buffer when needed.  We pre-size the Java buffer to
// 4 MiB for APE (via bytecode patch in FfmpegLibraryPatch.kt), so resizing is
// never triggered here.  The extra parameter is accepted but not used.
AUDIO_DECODER_FUNC(jint, ffmpegDecode, jlong context, jobject inputData,
                   jint inputSize, jobject outputBufferObj, jobject outputData,
                   jint outputSize) {
  if (!context) {
...
AVCodecContext *createContext(JNIEnv *env, AVCodec *codec, jbyteArray extraData,
  ...
  // Always pass sample_rate and channels so avcodec_open2 doesn't reject codecs
  // (like APE) that read them from extradata in init() but need them pre-set.
  // PCM_MULAW/ALAW are raw PCM and have no extradata, so they need this too.
  if (rawSampleRate > 0)
